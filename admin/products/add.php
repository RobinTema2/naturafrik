...
$categories = [];
try {
    $categories = db()->query('SELECT * FROM categories WHERE is_active=1 ORDER BY section, display_order')->fetchAll();
} catch (Exception $e) {}
        ...
        if (strlen($name) < 2 || $cat_id === 0) {
            ...
            if (!empty($_FILES['main_image']['name'])) {
                ...
                if ($uploaded) {
                    ...
                    $error = 'Erreur lors de l\'upload de l\'image. Vérifiez le format (JPG, PNG, WebP) et la taille (max 5MB).';
                }
            }
            ...
            if (!$error) {
                try {
                    ...
                    // Nettoyer la description (HTML basique autorisé)
                    $allowed_tags = '<p><br><b><strong><i><em><ul><ol><li><h2><h3><h4><span>';
                    $desc = strip_tags($desc, $allowed_tags);

                    $stmt = db()->prepare('
                        INSERT INTO products (name,slug,short_description,description,price,price_unit,currency,category_id,main_image,stock_quantity,min_order,weight,origin,certification,tags,is_featured,is_new,is_active)
                        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                    ');
                    $stmt->execute([$name,$slug,$short_desc,$desc,$price ?: null,$price_unit,$currency,$cat_id,$main_image,$stock,$min_order,$weight,$origin,$cert,$tags,$is_featured,$is_new,$is_active]);

                    admin_log('product_added', "Produit ajouté: $name (ID: ".db()->lastInsertId().")");
                    set_flash('success', "Produit \"$name\" ajouté avec succès !");
...
<head>
  ...
  <title>Ajouter Produit – NATURAFRIK Admin</title>
...
<aside class="admin-sidebar" id="adminSidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <div class="nav-group">
      ...
      <a href="index.php" class="sidebar-link"><i class="fas fa-box-open"></i> Produits</a>
      <a href="add.php" class="sidebar-link active"><i class="fas fa-plus-circle"></i> Ajouter un produit</a>
  ...
  <div class="sidebar-footer">
    <div class="admin-user-info">
      ...
      <div><div class="admin-name"><?= e($_SESSION['admin_name'] ?? '') ?></div><div class="admin-role"><?= e($_SESSION['admin_role'] ?? '') ?></div></div>
    </div>
...
<main class="admin-main">
  <header class="admin-topbar">
    ...
    <div class="topbar-title">
      ...
      <span><a href="index.php" style="color:var(--gold)">Produits</a> / Nouveau</span>
    </div>
        ...
        <div>
          <div class="admin-form-card" style="margin-bottom:20px;">
            ...
            <div class="form-group">
              ...
              <input type="text" name="name" class="form-control" placeholder="Ex: NATCACAO – Poudre de Cacao Pur 500g" required value="<?= e($_POST['name'] ?? '') ?>">
            </div>
            <div class="form-group">
              <label class="form-label">Description courte</label>
              <textarea name="short_description" class="form-control" rows="2" placeholder="Résumé en 1-2 phrases (affiché dans les listes)..."><?= e($_POST['short_description'] ?? '') ?></textarea>
            </div>
            <div class="form-group">
              <label class="form-label">Description complète (HTML accepté)</label>
              <textarea name="description" class="form-control" rows="8" placeholder="Description détaillée, notes de dégustation, fiche technique..."><?= e($_POST['description'] ?? '') ?></textarea>
            </div>
          </div>
          ...
          <div class="admin-form-card" style="margin-bottom:20px;">
            ...
            <div class="form-row">
              <div class="form-group">
                <label class="form-label">Prix (laisser vide si sur demande)</label>
                <input type="number" name="price" class="form-control" placeholder="5000" min="0" step="0.01" value="<?= e($_POST['price'] ?? '') ?>">
              </div>
              <div class="form-group">
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input type="text" name="price_unit" class="form-control" placeholder="kg, sachet 500g, tonne..." value="<?= e($_POST['price_unit'] ?? 'kg') ?>">
              </div>
              <div class="form-group">
                <label class="form-label">Stock disponible</label>
                <input type="number" name="stock_quantity" class="form-control" min="0" value="<?= e($_POST['stock_quantity'] ?? '0') ?>">
              </div>
            </div>
            <div class="form-row">
              <div class="form-group">
                <label class="form-label">Commande minimum</label>
                <input type="number" name="min_order" class="form-control" min="1" value="<?= e($_POST['min_order'] ?? '1') ?>">
              </div>
              <div class="form-group">
                <label class="form-label">Poids / Volume</label>
                <input type="text" name="weight" class="form-control" placeholder="500g, 1kg, 1 tonne..." value="<?= e($_POST['weight'] ?? '') ?>">
              </div>
            </div>
          </div>
          ...
          <div class="admin-form-card">
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input type="text" name="origin" class="form-control" placeholder="Région Ouest, Cameroun" value="<?= e($_POST['origin'] ?? '') ?>">
              </div>
              <div class="form-group">
                <label class="form-label">Certifications</label>
                <input type="text" name="certification" class="form-control" placeholder="Bio, ISO, HACCP..." value="<?= e($_POST['certification'] ?? '') ?>">
              </div>
            </div>
            <div class="form-group">
              <label class="form-label">Tags (séparés par des virgules)</label>
              <input type="text" name="tags" class="form-control" placeholder="cacao, cameroun, bio, premium..." value="<?= e($_POST['tags'] ?? '') ?>">
            </div>
          </div>
        </div>
        ...
        <div>
          <div class="admin-form-card" style="margin-bottom:20px;">
            ...
            <select name="category_id" class="form-select" required>
              ...
              <?php endforeach; ?>
              <?php if ($current_section) echo '</optgroup>'; ?>
            </select>
          </div>
          ...
          <div class="admin-form-card" style="margin-bottom:20px;">
            ...
            <div class="form-check">
              <input type="checkbox" name="is_active" id="isActive" class="form-check-input" <?= !empty($_POST) ? (isset($_POST['is_active']) ? 'checked' : '') : 'checked' ?>>
              <label for="isActive" class="form-check-label">Produit actif (visible sur le site)</label>
            </div>
            <div class="form-check">
              <input type="checkbox" name="is_featured" id="isFeatured" class="form-check-input" <?= isset($_POST['is_featured']) ? 'checked' : '' ?>>
              <label for="isFeatured" class="form-check-label">Produit vedette (affiché en premier)</label>
            </div>
            <div class="form-check">
              <input type="checkbox" name="is_new" id="isNew" class="form-check-input" <?= isset($_POST['is_new']) ? 'checked' : '' ?>>
...
function previewImg(input) {
  ...
  if (input.files?.[0]) {
    ...
    reader.onload = (e) => {
      preview.innerHTML = `<div class="img-preview-item"><img src="${e.target.result}" alt="Preview"></div>`;
    };
